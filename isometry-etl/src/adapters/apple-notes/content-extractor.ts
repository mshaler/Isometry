...
import { ParsedContent, AttachmentMeta } from '../../core/types.js';
...
function extractStringsFromProtobuf(data: Buffer, strings: string[]): void {
  ...
  while (offset < data.length) {
    try {
      ...
      const { value: header, bytesRead: headerBytes } = readVarint(data, offset);
      if (headerBytes === 0) break;
      
      offset += headerBytes;
      
      const fieldNumber = Number(header >> 3n);
 ...
 */
function readVarint(data: Buffer, offset: number): { value: bigint; bytesRead: number } {
  let result = 0n;
  let shift = 0n;
  let bytesRead = 0;
