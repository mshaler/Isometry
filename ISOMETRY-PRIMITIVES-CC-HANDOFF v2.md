...
Each Super* component has a **primary owner** that determines how it should be implemented. The litmus test: "Does this component manage application state (facet mappings, data mutations, undo) or presentation state (layout, visibility, highlighting)?"
...
**React's render boundary stops at the grid container edge.**
...
**3 of 5 Super* components are NOT React components.** SuperStack, SuperDensity (effect), and SuperCell should be implemented as D3 renderers that read CSS custom properties. They live in `src/renderers/`, not `src/components/`.

**2 of 5 Super* components ARE React components.** SuperDynamic and SuperFilter manage application state and live in `src/components/supergrid/`.

### Communication Pattern

React → D3 communication happens through a config object, not prop drilling into the grid:

```typescript
// React sets the config (application state)
interface SuperGridConfig {
  rowFacets: FacetConfig[];     // from SuperFilter
  colFacets: FacetConfig[];     // from SuperFilter
  axisMapping: AxisMapping;     // from SuperDynamic (after drag-drop)
}

// D3 reads CSS primitives (presentation state)
// D3 reads SuperGridConfig (application state)
// D3 renders everything inside the grid container

// The only React→D3 bridge:
gridRenderer.update(config);    // called when SuperFilter or SuperDynamic changes
```

React never reaches inside the grid to update individual headers or cells. D3 owns that entire DOM subtree.

### SuperDensity Detail

SuperDensity deserves extra explanation because it straddles the boundary:

```typescript
// React component: just a slider
function SuperDensitySlider({ onChange }) {
  return <input type="range" min={60} max={240} 
    onChange={e => {
      // Update CSS custom property directly — no React state for the grid
      document.documentElement.style.setProperty(
        '--iso-grid-cell-min-w', `${e.target.value}px`
      );
      // Optionally notify D3 if density mode needs to switch (count vs miniCard)
...
The slider is React. The visual effect is CSS. If density crosses a threshold (e.g., cells too small for mini-cards → switch to count display), D3 handles that transition via its data-join, not React.

### File Location Summary

```
src/
  components/supergrid/
    SuperDynamic.tsx              ← React: axis drag-drop repositioning
    SuperFilter.tsx               ← React: LATCH facet navigator
    SuperDensitySlider.tsx        ← React: thin slider, sets CSS vars
  renderers/
    SuperGridRenderer.ts          ← D3: orchestrates all rendering
    SuperStackRenderer.ts         ← D3: nested header rendering
    SuperCellRenderer.ts          ← D3: data cell rendering
  styles/
    primitives-supergrid.css      ← CSS: all SuperGrid layout primitives
    chrome/
      sticky-headers.css          ← CSS: sticky header behavior
      selection.css               ← CSS: selection highlighting
      scroll-shadows.css          ← CSS: scroll indicators
```
