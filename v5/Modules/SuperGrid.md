...
Layer 4 (Control Chrome):    FilterNav, density sliders, axis pickers, toolbars
Layer 2 (sql.js):            LATCH queries → JSON → D3 data binding
```
  
  
**Rendering rule:** D3 shows the truth, Control Chrome lets you change it.  
⸻  
## 2. Feature Catalog  
...
⸻  
### 2.5 SuperZoom — Unified Aggregation Control  
  
**What:** The Janus density model — a unified control for semantic zoom across LATCH dimensions. Density/sparsity zoom is aggregation level, not just visual compactness.  
...
**Why:** With stacked headers and overlapping z-layers (D3 sparsity layer, Control Chrome density layer, overlay layer), selection needs to know which layer the user is targeting.  
...
**What:** SuperStack header cards, SuperAudit overlay cards, and aggregation cards are "generated" cards distinct from user data cards.   
...
- Formula bar appears in Control Chrome chrome, results render in D3 data plane  
...
| Formula update | Change Jan value | Q1 sum automatically recalculates | Control Chromeive update < 100ms |
...
**Why:** Time is the most common LATCH axis and the hardest to get right. Automatic parsing of date formats, smart header generation (Year → Quarter → Month → Week → Day), and the ability to select non-contiguous time ranges (Q1 + Q3, skipping Q2) are essential. D3.js’ Time library may serve as helpful here: [https://d3js.org/d3-time](https://d3js.org/d3-time)  
...
**Behavior:**  
- Search bar in Control Chrome chrome  
...
**Key constraint:** D3's enter/update/exit IS state management. No Redux, no Zustand. Chrome controls dispatch intents; D3 executes them.  
⸻  
...
- [ ] **Quantrix-style formula model:** Review [https://help.idbs.com/Quantrix/Modeler_Help/LATEST/en/working-with-formulae.html](https://help.idbs.com/Quantrix/Modeler_Help/LATEST/en/working-with-formulae.html) for dimensional formula patterns.  
