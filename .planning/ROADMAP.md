...
**Depends on**: Phase 135
**Requirements**: TIME-01, TIME-02, TIME-04, TIME-05, TIME-06
**Success Criteria** (what must be TRUE):
  1. SuperGrid with a time column on the row or column axis produces GROUP BY strftime('%Y-%m', field) SQL when granularity is 'month' -- Vitest confirms SQL output for all 5 granularities (day/week/month/quarter/year)
  2. When granularity is null and a time axis is present, SuperGrid defaults to month-level bucketing -- raw ISO timestamps never appear as SuperGrid header labels
...
**Goal**: SuperGrid headers display human-readable time labels and users can switch granularity from the Workbench panel when a time axis is active
**Depends on**: Phase 136
**Requirements**: TIME-03, TVIS-01, TVIS-02
**Success Criteria** (what must be TRUE):
...
**Goal**: FilterProvider supports time range filtering on any time column, with axis projection and membership filtering fully independent, and multi-field OR-semantics membership filtering available
**Depends on**: Phase 136
...
**Plans**: 1 plan
Plans:
- [x] 141-01-PLAN.md — Data attributes, pointer event bridge, afterRender rootEl fix, user-select CSS
**UI hint**: no
