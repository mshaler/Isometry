...
4. **Error handling:**
   ...
   - Load failed command into input for retry editing:
     ```typescript
       ...
       setCommandInput(command.finalCommand);
       setIsExecuting(false);
     }
     ```

5. **Cmd+K keyboard shortcut for command palette:**
   - Add useEffect in GSDInterface.tsx to listen for keyboard shortcut:
     ```typescript
     useEffect(() => {
     ...
     ```
   - Pass `commandPaletteOpen` and `setCommandPaletteOpen` props to RichCommandBuilder
   - **Command palette UI behavior:** When `commandPaletteOpen` becomes true, RichCommandBuilder:
     - Shows the command input field prominently (expands from collapsed state if minimized)
     - Auto-focuses the command input field for immediate typing
     - Displays a filterable list of available GSD commands below the input
     - User types to filter commands, arrow keys to navigate, Enter to select
     - Escape or clicking outside closes the palette (sets `commandPaletteOpen` to false)
...
- Failed commands: load into input for editing before retry (setCommandInput call)
  </action>
  <verify>
...
5. **Auto-scroll with user scroll detection (in ClaudeCodeTerminal.tsx only):**
   - **Single source of truth:** ClaudeCodeTerminal.tsx owns all scroll behavior. ExecutionProgress.tsx only displays phase status and activity indicator — no scroll logic there.
   - Track scroll position in terminal output container:
     ```typescript
     // In ClaudeCodeTerminal.tsx
     ...
     // Auto-scroll on new output (unless user scrolled up)
     useEffect(() => {
       if (!isUserScrolledUp && outputRef.current) {
         outputRef.current.scrollTop = outputRef.current.scrollHeight;
       }
     }, [outputContent, isUserScrolledUp]);
     ```
...
- Auto-scroll follows output until user scrolls up, then holds position (resume at bottom)
  </action>
  <verify>
Run `npm run check:types` to verify no TypeScript errors.
...
4. Verify Cmd+K opens command palette
5. Verify activity indicator animates during execution
6. Verify auto-scroll follows output until user scrolls up
7. Verify auto-scroll resumes when user scrolls to bottom
8. Verify toast notification appears on task completion
9. Verify failed commands load into input for retry editing
10. Verify tool calls display collapsed with expand toggle
</verification>
...
- Auto-scroll follows output until user scrolls up, resumes when at bottom
- Toast notification appears on task completion
- Failed commands load into input for retry editing
