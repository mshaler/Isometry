...
must_haves:
  truths:
    ...
    # Note: "View transitions are smooth" is a phase-level requirement verified across all tabs, not specific to timeline
  artifacts:
...
First, verify that 44-01 and 44-02 created their components (this plan depends on them):
```bash
# Verify dependencies from 44-01 and 44-02 exist
ls src/components/notebook/preview-tabs/NetworkGraphTab.tsx && echo "NetworkGraphTab exists" || echo "ERROR: NetworkGraphTab.tsx missing - 44-01 incomplete"
ls src/components/notebook/preview-tabs/DataInspectorTab.tsx && echo "DataInspectorTab exists" || echo "ERROR: DataInspectorTab.tsx missing - 44-02 incomplete"
```

If either file is missing, STOP and report the issue - the dependent plans must complete first.

Then create the barrel export:
```typescript
