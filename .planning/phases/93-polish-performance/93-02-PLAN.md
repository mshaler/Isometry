...
<task type="auto">
  <name>Task 3: Integrate empty state and accessibility into SuperGrid</name>
  <files>src/components/supergrid/SuperGrid.tsx</files>
  <action>
Update SuperGrid.tsx to use the new empty state and accessibility components:
...
2. Replace the existing empty state (around line 571-580) with the new component:
```typescript
// Empty state - determine type based on context
if (!nodes?.length) {
  // Determine empty state type
  let emptyType: EmptyStateType = 'first-use';

  // Check if filters are active (would indicate 'no-results')
...
3. Wrap the main grid content with SuperGridAccessibility (around line 583-767):
...
4. Update the error state (around line 556-569) to use new component:
```typescript
if (error) {
  return (
...
5. Add ARIA roles to existing elements:
- Add `role="row"` to header row containers
- Add `role="columnheader"` to column headers
- Add `role="rowheader"` to row headers
- Add `role="gridcell"` to data cells
- Add `aria-colindex` and `aria-rowindex` to cells

These changes integrate accessibility without restructuring the component.
  </action>
  <verify>
Run `npm run check:types` to verify no TypeScript errors.
Run `npm run dev` and verify:
1. Empty state shows "No Data Yet" when no nodes
2. Empty state shows "No Matching Results" when filters active but no matches
3. Error state shows "Failed to Load Data" with SQL query
4. Arrow keys navigate between grid cells (check with DevTools focused element)
5. Screen reader announces "grid with X rows, Y columns" (test with VoiceOver on Mac)
  </verify>
  <done>
- SuperGrid uses SuperGridEmptyState for empty/error states
- SuperGrid wrapped with SuperGridAccessibility
- ARIA roles added to headers and cells
...
3. Arrow key navigation moves focus between cells
4. Screen reader announces grid structure (rows, columns)
