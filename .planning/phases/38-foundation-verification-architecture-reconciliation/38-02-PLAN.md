...
must_haves:
  ...
  artifacts:
    ...
    - path: "src/d3/SuperGrid.ts"
      provides: "Direct sql.js Database usage via useDatabaseService hook"
      contains: "useDatabaseService"
    - path: "src/components/SuperGridDemo.tsx"
      provides: "Adapter pattern removed, direct hook usage implemented"
      min_lines: 300
  key_links:
    - from: "SuperGrid class constructor"
      to: "sql.js Database instance"
      via: "useDatabaseService hook"
      pattern: "const.*database.*=.*useDatabaseService"
    - from: "Demo components"
      to: "sql.js Database instance"
      via: "useDatabaseService hook direct access"
      pattern: "useDatabaseService.*db"
...
<task type="auto">
  <name>Update SuperGrid to Use Unified Hook</name>
  <files>src/d3/SuperGrid.ts</files>
  <action>
    Modify SuperGrid class to use the unified useDatabaseService hook instead of requiring DatabaseService class injection.

    Key changes:
    1. Remove DatabaseService import and constructor parameter
    2. Add useDatabaseService hook usage
    3. Update all database access to use hook return values
    ...
    export class SuperGrid {
      ...
      constructor(
        ...
        this.database = useDatabaseService();
        // ... rest unchanged
      }
    ```
    ...
    The key architectural change is eliminating the DatabaseService class dependency.

    Note: This requires SuperGrid to be called within a React component context that has SQLiteProvider.
  </action>
  <verify>grep -c "useDatabaseService" src/d3/SuperGrid.ts should return 2 (import and usage)</verify>
  <done>SuperGrid class updated to use unified database access hook, eliminating DatabaseService class dependency</done>
</task>
...
<task type="auto">
  ...
  <action>
    ...
    3. Update SuperGrid initialization to not pass database parameter:
    ```typescript
    // CHANGE FROM:
    const grid = new SuperGrid(container, dbService, config, callbacks);

    // TO:
    const grid = new SuperGrid(container, config, callbacks);
    ```

    4. Remove dbService dependency from useEffect:
    ```typescript
    // Update the useEffect that creates SuperGrid to remove dbService dependency
    // and instead depend on db being available
    ```

    5. Update imports - remove DatabaseService import if no longer used:
    ```typescript
    ...
    All functionality should work the same because SuperGrid now gets database access via hook internally.
  </action>
  <verify>grep -c "adapter" src/components/SuperGridDemo.tsx should return 0 (all adapter code removed)</verify>
  <done>Demo component adapter patterns removed, SuperGrid initialization simplified to use unified hook approach</done>
</task>
...
<task type="auto">
  ...
  <action>
    ...
    3. Update SuperGrid instantiation to remove database parameter

    For src/components/supergrid/SuperGrid.tsx:
    1. Check if this is a wrapper component that needs similar updates
    2. Apply same pattern - remove adapters, use hook-based approach
    ...
    3. Direct hook usage in components
  </action>
  <verify>grep -r "adapter.*DatabaseService\|DatabaseService.*adapter" src/components/ should return no matches</verify>
  <done>All adapter patterns eliminated from components, unified database access approach implemented throughout codebase</done>
</task>
...
2. SuperGrid class uses hook instead of DatabaseService class dependency
3. Demo components have adapter code removed and simplified instantiation
...
- SuperGrid and demo components access Database directly via hook
