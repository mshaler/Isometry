...
3. Performance benchmark test (SQL performance target):
   - Setup: Generate 10,000 mock nodes in test database
   - Test: Execute buildHeaderDiscoveryQuery for simple, date, and multi_select facets
   - Assert: Each query completes in <100ms (using `performance.now()`)
   - Note: This validates "Query completes in <100ms for 10K cards" success criterion
