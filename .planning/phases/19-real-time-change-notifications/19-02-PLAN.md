...
must_haves:
  ...
  key_links:
    ...
    - from: "useOptimisticUpdates"
      to: "useLiveQuery"
      via: "rollback reconciliation"
      pattern: "rollback.*reconcile.*liveQuery"
...
<task type="auto">
  ...
  <action>
    ...
    1. Create useOptimisticUpdates hook:
       ...
       - CRITICAL: Integrate with useLiveQuery for actual data state reconciliation during rollback scenarios
       - On rollback: restore actual data from useLiveQuery current state, not cached optimistic state
       - Handle rollback conflicts when live data has changed during optimistic operation
       - Implement reconciliation strategy: merge optimistic changes with live data where possible
       - Add rollback event emission to notify other hooks of state changes
  ...
  <verify>npm run build && echo "TypeScript compilation successful with optimistic updates and connection management" && node -e "
    const fs = require('fs');
    const hookPath = '/Users/mshaler/Developer/Projects/Isometry/src/hooks/useOptimisticUpdates.ts';
    if (fs.existsSync(hookPath)) {
      const content = fs.readFileSync(hookPath, 'utf8');
      if (content.includes('useLiveQuery') && content.includes('reconcile')) {
        console.log('✓ Optimistic updates rollback integration with useLiveQuery implemented');
      } else {
        console.log('⚠ Missing optimistic updates integration with useLiveQuery');
      }
    }
  "</verify>
  <done>useOptimisticUpdates provides instant UI updates with rollback reconciled against useLiveQuery state, ConnectionManager tracks bridge connectivity, ConnectionContext coordinates global connection state</done>
</task>
...
<task type="checkpoint:human-verify" gate="blocking">
  ...
  <how-to-verify>
    ...
    4. Test optimistic rollback scenarios:
       - Edit a card, then quickly trigger a failure condition
       - Verify rollback restores correct state from live query data
       - Confirm no stale optimistic state remains after rollback
    ...
    7. Monitor performance dashboard for real-time sync metrics
    Expected behavior: Instant UI updates, sub-100ms change propagation, clean rollback reconciliation, clear offline/online status, automatic conflict resolution
  </how-to-verify>
...
6. Optimistic update rollback properly reconciles with live query state
</verification>
...
6. Optimistic update rollbacks reconcile correctly with live query results
</success_criteria>
