...
must_haves:
  ...
  key_links:
    ...
    - from: "src/hooks/useGSDFileSync.ts"
      ...
      via: "imports GSDFileChangeEvent type"
      pattern: "import.*GSDFileChangeEvent"
...
import type { GSDFileChangeEvent } from '../services/gsd/gsdFileWatcher';
      ...
      try {
        ...
        switch (message.type) {
          ...
          case 'gsd_file_update': {
            // Handle GSDFileChangeEvent from gsdFileWatcher.ts (87-02)
            const fileEvent = message as GSDFileChangeEvent;
            // Invalidate queries for the changed file
            queryClient.invalidateQueries({
              queryKey: ['gsd-plan', fileEvent.filePath]
            });
...
1. Check that `gsd_file_update` message type is handled in the switch statement
2. Verify `GSDFileChangeEvent` type is imported from 87-02 artifacts
3. Verify `queryClient.invalidateQueries` is called on file update messages
  </verify>
  <done>GSD sync hooks created with WebSocket subscription, GSDFileChangeEvent handling, and optimistic updates</done>
</task>
...
- [ ] useGSDFileSync handles `gsd_file_update` messages (GSDFileChangeEvent from 87-02)
...
- WebSocket receives gsd_file_update messages and invalidates correct queries
