...
Output: `gsdConflictResolver.ts` detection logic and `GSDConflictModal.tsx` resolution UI.
</objective>
...
<task type="auto">
  <name>Task 2: Create conflict resolution modal component</name>
  <files>
    src/components/shell/GSDConflictModal.tsx
    src/components/shell/index.ts
  </files>
...
Note: Uses shadcn/ui Dialog and Button components. If not available, the component structure is correct - just needs the right imports adjusted.
  ...
  <verify>
Run `npm run typecheck` - no errors in GSDConflictModal.tsx.
  </verify>
  <done>Conflict modal component created with diff display and resolution buttons</done>
</task>
