...
7. Rename node_properties table definition to card_properties:
   - If node_properties table exists in schema.sql, rename to card_properties
   - Update node_id FK reference comment to indicate it now points to cards.id
   - If node_properties is not in schema.sql (migration-only), add a comment block:
     ```sql
     -- card_properties table: Created via migration from node_properties
     -- See: src/db/migrations/84-cards-connections.sql
     ```
...
7. Verify notebook_cards still references cards correctly:
   ```sql
   -- Verification query (add as comment in migration script)
   -- After migration, this must return rows:
   -- SELECT COUNT(*) FROM notebook_cards n JOIN cards c ON n.node_id = c.id;
   -- notebook_cards.node_id column name stays the same but now references cards.id
   ```
  </action>
...
describe('Cards & Connections Migration', () => {
  ...
  it('should verify notebook_cards can join with cards table', () => {
    // Per research decision #4: notebook_cards stays as-is, referencing cards.id via node_id column
    db.run(`
      CREATE TABLE cards (
        id TEXT PRIMARY KEY,
    ...
    db.run(`
      CREATE TABLE notebook_cards (
        id TEXT PRIMARY KEY,
        node_id TEXT NOT NULL,  -- References cards.id after migration
        markdown_content TEXT
      )
    `);

    // Insert test data
    db.run("INSERT INTO cards VALUES ('c1', 'note', 'Test Card')");
    db.run("INSERT INTO notebook_cards VALUES ('nc1', 'c1', '# Hello')");

    // Verify JOIN works (critical for post-migration integrity)
    const result = db.exec(`
      SELECT n.id, n.node_id, c.name, c.card_type
      FROM notebook_cards n
      JOIN cards c ON n.node_id = c.id
    `);

    expect(result[0].values).toHaveLength(1);
    expect(result[0].values[0]).toEqual(['nc1', 'c1', 'Test Card', 'note']);
  });
});
```
