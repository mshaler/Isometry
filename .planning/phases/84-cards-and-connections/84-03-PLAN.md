...
must_haves:
  truths:
    ...
    - "Filter compiler uses cards.* table references not nodes.*"
  ...
  artifacts:
    - path: "src/hooks/database/useLiveQuery.ts"
      ...
      contains: "FROM cards"
    - path: "src/filters/compiler.ts"
      provides: "LATCH filter SQL compilation"
      contains: "cards.folder"
  ...
  key_links:
    ...
    - from: "src/filters/compiler.ts"
      to: "src/db/schema.sql"
      via: "cards table references"
      pattern: "cards\\.(folder|status|tags|priority)"
...
<task type="auto">
  <name>Task 1: Update database hooks and filters to use cards table</name>
  <files>src/hooks/database/useLiveQuery.ts, src/hooks/database/useFTS5Search.ts, src/hooks/database/useLiveQueryCore.ts, src/filters/compiler.ts, src/filters/index.ts</files>
  <action>
Update the database hooks and filter compiler to query cards instead of nodes:
...
1. **useLiveQuery.ts** - Update main query hook:
   ...
   - Update filter references: `nodes.folder` -> `cards.folder`, `nodes.created_at` -> `cards.created_at`, etc.

1b. **src/filters/compiler.ts** - Update filter SQL compilation:
   - Update all table references: `nodes.folder` -> `cards.folder`
   - Update all table references: `nodes.status` -> `cards.status`
   - Update all table references: `nodes.tags` -> `cards.tags`
   - Update all table references: `nodes.priority` -> `cards.priority`
   - Update all table references: `nodes.created_at` -> `cards.created_at`
   - Update all table references: `nodes.modified_at` -> `cards.modified_at`
   - Update all table references: `nodes.due_at` -> `cards.due_at`
   - Update all table references: `nodes.event_start` -> `cards.event_start`
   - Update all table references: `nodes.event_end` -> `cards.event_end`
   - Update all table references: `nodes.location_name` -> `cards.location_name`

1c. **src/filters/index.ts** - Update any re-exported types or functions referencing Node
...
2. grep "FROM cards" src/hooks/database/*.ts shows updated queries
3. grep -r "nodes\\." src/filters src/hooks --include="*.ts" returns 0 matches (all table refs updated)
</verify>
