...
# Verify HeaderLayoutService integration (key_links verification)
grep -n "generateStackedHierarchy" src/d3/grid-rendering/GridRenderingEngine.ts
# Verify import wiring is complete
grep -n "import.*HeaderLayoutService" src/d3/grid-rendering/GridRenderingEngine.ts
```
  </verify>
  <done>GridRenderingEngine detects stacked axes and delegates to SuperGridHeaders.renderStackedHeaders. HeaderLayoutService import verified. Card positions computed from leaf nodes of hierarchy.</done>
</task>

<!-- NOTE: GridRenderingEngine.ts is currently 910 lines. Adding ~80 lines brings it to ~990 lines,
     approaching the 1000-line threshold. If this plan causes the file to exceed limits, a follow-up
     refactoring task should extract stacked header rendering into a separate module. -->
