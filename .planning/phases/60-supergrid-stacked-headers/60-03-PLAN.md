...
<!-- NOTE: Task 1 (PAFVContext changes) could theoretically run in parallel with Plan 60-02 (Wave 2)
     since state changes don't depend on rendering. However, keeping tasks sequential in Wave 3
     simplifies execution and avoids merge conflicts in shared type files. This is an acceptable
     trade-off for a solo developer workflow. -->
...
Manual verification:
1. Run `npm run dev`
2. Navigate to `/?test=integrated`
3. Click on a header cell - verify sort indicator appears
4. Click again - verify direction toggles (asc -> desc -> none)
5. Verify hover states work on headers
  </verify>
  <done>SuperGridHeaders has handleHeaderSortClick method. Sort state toggles through asc/desc/null. Header click events include sortDirection. Manual verification confirms interaction works.</done>
</task>
