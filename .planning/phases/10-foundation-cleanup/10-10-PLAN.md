...
<task type="auto">
  ...
  <verify>npm run lint | grep -E "(any|typescript-eslint/no-explicit-any)" | wc -l returns 0</verify>
  <done>All 10 explicit any type violations eliminated with proper TypeScript interfaces</done>
</task>

<task type="auto">
  <name>Task 2: Clean up unused variables across components</name>
  <files>src/components/notebook/NotebookContext.tsx, src/components/shell/CommandHistory.tsx, src/components/visualization/VisualizationCanvas.tsx, multiple component files</files>
  <action>
    Based on verification gaps, eliminate the 11 unused variable warnings:

    Identify all unused variables with: npm run lint | grep "no-unused-vars"
    ...
    Focus on the known problem files from verification and run incremental lint checks.
    Maintain all existing functionality while eliminating warnings.
  </action>
  <verify>npm run lint | grep "no-unused-vars" | wc -l returns 0</verify>
  <done>All 11 unused variable warnings eliminated without breaking functionality</done>
</task>

<task type="auto">
  <name>Task 3: Validate absolute zero ESLint achievement</name>
  <files></files>
  <action>
    ...
    3. Run type check to ensure no regressions: npm run type-check
  ...
  <verify>npm run lint outputs exactly "✖ 0 problems (0 errors, 0 warnings)"</verify>
  <done>Phase 10 "absolute zero lint problems" goal completely achieved with measurable proof</done>
</task>
