# Phase 162 Handoff: SuperWidget Substrate

**Target path on commit:** `.planning/milestones/v13.0-superwidget-substrate/162-00-HANDOFF.md`

**Supersedes:** `145-00-HANDOFF.md` (r1), `145-00-HANDOFF-r2-revision-note.md`
...
- SuperWidget follows the established component pattern: TypeScript class with `mount(container: HTMLElement)` / `destroy()` lifecycle, matching WorkbenchShell, DataExplorerPanel, and all Explorer panels.

**Deliverables:**

1. `src/superwidget/SuperWidget.ts` — the substrate component (TypeScript class with `mount(container)` / `destroy()` lifecycle).
...
**Red-first gate:** Write `tests/superwidget/SuperWidget.substrate.test.ts` asserting that a rendered SuperWidget has exactly four slot elements with the correct `data-slot` attributes, and that the config gear exists within the tabs slot with `data-tab-role="config"`. Test must fail before implementation.

**Success criteria (grep-able):**
- `grep -r "data-slot=\"header\"" src/superwidget` — one primary match in component.
- `grep -r "data-slot=\"header\"" tests/superwidget` — multiple matches in tests.
...
- `grep -rn "data-tab-role=\"config\"" src/superwidget` — present in component.
- `grep -rn "data-tab-role=\"config\"" tests/superwidget` — present in tests.
...
**Red-first gate:** Write `tests/superwidget/projection.test.ts` with table-driven cases for each transition function, including invalid inputs. Tests must fail before implementation.
...
**Red-first gate:** Write `tests/superwidget/SuperWidget.projection.test.ts` asserting:
...
- `grep -rn "data-render-count" src/superwidget` — present in stubs (WA-4).
- `grep -rn "data-render-count" tests/superwidget` — referenced in tests.
...
1. `src/superwidget/canvas/ExplorerCanvasStub.ts`
   ...
   - Unbound only.
   - Does not populate the status slot. Real Data Explorer Canvas in v13.1 will emit ingestion count text into the status slot; the stub leaves it empty to preserve the zero-height invariant.
...
**Red-first gate:** Write `tests/superwidget/CanvasStubs.test.ts` asserting each stub renders with correct `data-canvas-type`, correct `canvasId` text, correct binding behavior, and that the View stub's Bound mode renders a sidecar element. Tests must fail before implementation.

**Success criteria (grep-able):**
- `grep -rn "data-canvas-type=\"Explorer\"" src/superwidget/canvas` — one match in stub.
- `grep -rn "data-canvas-type=\"Explorer\"" tests/superwidget` — referenced in tests.
- `grep -rn "data-canvas-type=\"View\"" src/superwidget/canvas` — same pattern.
...
- **v13.1:** Real Data Explorer Canvas (Integrate zone default) + real Projection Explorer sidecar + status slot first tenant (ingestion counts).
...
| Component pattern | TypeScript class with `mount(container)` / `destroy()` | Matches WorkbenchShell, DataExplorerPanel, all Explorer panels — established codebase convention |
| Test path convention | All tests under `tests/superwidget/` | Matches codebase convention — no colocated tests in `src/` |
...
This project uses dual numbering:
- **Sequential phase numbers** (1, 2, ... 161, 162, ...) — monotonically increasing across all milestones.
- **Versioned milestone groupings** (v12.0, v13.0, v13.1, ...) — each milestone contains one or more phases.

v12.0 Explorer Panel Polish used phases 155–161. This milestone (v13.0) starts at phase 162. Subsequent Canvas milestones (v13.1, v13.2, ...) continue the sequence (163, 164, ...). No v12.x sub-milestones remain.
...
| Rev | Date | Changes |
|---|---|---|
| r1 | 2026-04-21 | Initial handoff (filed as 145-00-HANDOFF.md) |
| r2 | 2026-04-21 | Renumbered Phase 145 → 162. Status slot: named Integrate ingestion counts as first tenant (v13.1). Pinned component pattern (TS class, mount/destroy). Fixed all test paths from `src/` to `tests/superwidget/`. Split grep criteria to separate src and test paths. Added numbering convention section. |

---

*Authored: 2026-04-21*
*Phase numbering: continues from v12.0 Phase 161*
