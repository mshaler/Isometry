...
- [ ] **EDIT-01**: User can edit a card's title inline in the Notebook panel and changes persist to cards.name
- [ ] **EDIT-02**: User can edit a card's content in the Notebook panel and changes persist to cards.content (not ui_state)
- [ ] **EDIT-03**: Notebook uses shadow-buffer architecture — snapshot on card load, mutable buffer during edit, single MutationManager mutation on commit (blur/switch/save)
- [ ] **EDIT-04**: All card edits support undo/redo via MutationManager (Cmd+Z / Cmd+Shift+Z)
...
- [ ] **EDIT-06**: Card edits trigger CloudKit sync via existing dirty flag pipeline (mutated changeset dispatch)
- [ ] **EDIT-07**: Notebook resets to idle state when the active card is deleted by undo (MutationManager subscriber safety)
