...
export class TreeView implements IView {
  ...
  mount(container: HTMLElement): void {
    ...
    this.treeLayer = this.svg
      ...
      .attr('transform', `translate(${TREE_ORIGIN_X}, ${TREE_ORIGIN_Y})`) as d3.Selection<SVGGElement, unknown, null, undefined>;

    // Sub-groups: links below nodes
    this.linksGroup = this.treeLayer.append('g').attr('class', 'tree-links') as d3.Selection<SVGGElement, unknown, null, undefined>;
    this.nodesGroup = this.treeLayer.append('g').attr('class', 'tree-nodes') as d3.Selection<SVGGElement, unknown, null, undefined>;
  ...
  async render(cards: CardDatum[]): Promise<void> {
    ...
    const existingNodeIds = this.root
      ? new Set(this.root.descendants().map(n => n.data.id).filter(id => id !== '__forest_root__'))
      : null;
  ...
  destroy(): void {
    ...
    if (this.orphanContainer && this.orphanContainer.parentNode) {
  ...
  private _updateTreeRender(): void {
    ...
    const visibleLinks = this.root.links().filter(
      l => l.source.data.id !== '__forest_root__' && l.target.data.id !== '__forest_root__'
    );
    ...
    this.linksGroup
      ...
      .join(
        ...
        update =>
          update.attr('d', linkPathGenerator as unknown as (d: d3.HierarchyLink<TreeNodeData>) => string),
        exit => exit.remove()
      );
  ...
  private _renderOrphans(orphanCards: CardDatum[]): void {
    ...
    header.style.cssText = 'font-size:12px;font-weight:600;color:#888;padding:8px 12px 4px;border-top:1px solid #e0e0e0;margin-top:8px;';
