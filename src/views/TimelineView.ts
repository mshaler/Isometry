...
export class TimelineView implements IView {
  ...
  mount(container: HTMLElement): void {
    ...
    this.axisG = this.svg
      ...
      .attr('transform', `translate(${LABEL_COL_WIDTH}, ${AXIS_HEIGHT})`) as d3.Selection<SVGGElement, unknown, null, undefined>;

    // Swimlane container group — positioned below the axis
    this.swimlaneG = this.svg
      .append('g')
      .attr('class', 'swimlanes')
      .attr('transform', `translate(${LABEL_COL_WIDTH}, ${AXIS_HEIGHT + 10})`) as d3.Selection<SVGGElement, unknown, null, undefined>;
  ...
  render(cards: CardDatum[]): void {
    ...
    const grouped = d3.group(
      timeCards,
      c => String((c as unknown as Record<string, unknown>)[this.groupByField] ?? 'None')
    );
    ...
    for (const [label, laneCards] of grouped) {
      ...
      const xPositions = new Map<string, number>(
        laneCards.map(c => [c.id, xScale(new Date(c.due_at!))])
      );
      ...
      const sortedXPositions = new Map<string, number>(
        sorted.map(c => [c.id, xScale(new Date(c.due_at!))])
      );
