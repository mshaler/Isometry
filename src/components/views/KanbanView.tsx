...
export function KanbanView({ data, groupBy = 'status', onCardClick }: KanbanViewProps) {
  ...
  const { columns, grouped } = useMemo(() => {
    ...
    data.forEach(card => {
      let value: string;
      if (groupBy === 'status') {
        value = card.status || 'Unknown';
      } else if (groupBy === 'category') {
        value = card.category || 'Unknown';
      } else {
  ...
  useEffect(() => {
    ...
    columnsSelection.each(function(columnKey) {
      ...
      const cards = grouped.get(columnKey) || [];
      ...
      column.append('div')
        ...
        .html(`
          <div class="flex items-center justify-between">
            <span>${columnKey}</span>
            <span class="text-xs ${theme === 'NeXTSTEP' ? 'text-[#505050]' : 'text-gray-400'}">${cards.length}</span>
          </div>
        `);

      // Cards container
      const cardsContainer = column.append('div')
        .attr('class', 'flex-1 p-2 space-y-2 overflow-y-auto')
        .style('max-height', '400px');

      // Cards
      const cardElements = cardsContainer
        .selectAll<HTMLDivElement, CardData>('.kanban-card')
        .data(cards, d => d.id)
        .join(
          enter => enter
            .append('div')
      ...
      cardElements
        .attr('class', `kanban-card cursor-pointer ${
          theme === 'NeXTSTEP'
      ...
      cardElements.each(function(card) {
        const cardEl = d3.select(this);
        cardEl.selectAll('*').remove();
        ...
        if (card.content) {
          cardEl.append('div')
            ...
            .text(card.content.slice(0, 80) + (card.content.length > 80 ? '...' : ''));
        }

        // Footer with badges
        const footer = cardEl.append('div')
          .attr('class', 'flex items-center gap-2');

        if (groupBy !== 'category' && card.category) {
          footer.append('span')
  ...
  }, [columns, grouped, theme, groupBy, onCardClick]);
