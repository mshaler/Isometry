 ...
 *
 * IMPORTANT: This adapter requires a Node.js runtime (Tauri backend, Electron, or CLI).
 * It uses better-sqlite3 which is a native module and cannot run in the browser.
 *
 * Features:
...
import { extractNoteContent } from './content-extractor';
...
export class AppleNotesAdapter implements SourceAdapter {
  ...
  async fullSync(): Promise<SyncResult> {
    ...
    try {
      ...
      for (const row of rows) {
        ...
        const folderEdges = this.createFolderStructure(
          row,
          node.id,
          folderNodes
        );
      ...
      // Update sync state
      const countResult = this.db!.prepare(QUERY_NOTE_COUNT).get() as {
        count: number;
      };
      this.syncState = {
        lastSync: new Date(),
  ...
  async incrementalSync(state: SyncState): Promise<SyncResult> {
    ...
    try {
      ...
      const rows = this.db!.prepare(QUERY_NOTES_SINCE).all(
        watermark
      ) as RawNoteRow[];
      ...
      // Query deleted notes
      const deletedRows = this.db!.prepare(QUERY_DELETED_NOTES).all(
        watermark
      ) as { id: number }[];
  ...
  private async rowToCanonicalNode(row: RawNoteRow): Promise<CanonicalNode> {
    ...
    try {
      const contentRow = this.db!.prepare(QUERY_NOTE_CONTENT).get(row.id) as
        | RawContentRow
        | undefined;
