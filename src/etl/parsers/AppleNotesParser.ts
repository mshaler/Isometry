...
import type { AltoNoteFrontmatter, CanonicalCard, CanonicalConnection, ParseError } from '../types';
import { extractHashtags, extractMentions, extractNoteLinks, parseTableToMarkdown } from './attachments';
...
export class AppleNotesParser {
	 ...
	 */
	private parseFile(file: ParsedFile, index: number): { cards: CanonicalCard[]; connections: CanonicalConnection[] } {
		const cards: CanonicalCard[] = [];
		const connections: CanonicalConnection[] = [];
