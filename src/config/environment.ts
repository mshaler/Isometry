...
/**
 * Clears cached configuration (for testing)
 */
...
/**
 * Gets secure temporary directory path
 */
export function getSecureTempDirectory(): string {
  const config = getEnvironmentConfig();
  ...
  // Secure fallbacks
  if (config.isDevelopment) {
    return '/tmp/isometry-temp';
  }

  throw new Error(
    'Temporary directory not configured. Set VITE_TEMP_DIRECTORY or TEMP_DIRECTORY environment variable.'
  );
}

/**
 * Gets secure cache directory path
 */
export function getSecureCacheDirectory(): string {
  const config = getEnvironmentConfig();
  ...
  // Secure fallbacks
  if (config.isDevelopment) {
    return '/tmp/isometry-cache';
  }

  throw new Error(
    'Cache directory not configured. Set VITE_CACHE_DIRECTORY or CACHE_DIRECTORY environment variable.'
  );
}
 ...
 */
export function validateEnvironmentSecurity(): { valid: boolean; errors: string[]; warnings: string[] } {
  const config = getEnvironmentConfig();
  const errors: string[] = [];
  ...
  if (config.isProduction) {
    if (!config.enableAPIProxy) {
      errors.push('API proxy must be enabled in production');
    }
    ...
    if (config.logLevel === 'debug') {
      warnings.push('Debug log level should not be used in production');
    }
  ...
  if (config.enableExperimentalFeatures && config.isProduction) {
    warnings.push('Experimental features enabled in production');
  }
...
/**
 * Gets environment-specific storage key
 */
export function getEnvironmentStorageKey(key: string): string {
  const config = getEnvironmentConfig();
  return `${config.storagePrefix}:${config.type}:${key}`;
}

/**
 * Gets feature flag status
 */
export function isFeatureEnabled(feature: keyof Pick<EnvironmentConfig,
  'enableNotebookMode' | 'enableClaudeIntegration' | 'enableOfflineMode' | 'enableExperimentalFeatures'
>): boolean {
  const config = getEnvironmentConfig();
