...
describe('ETL All Parsers Integration', () => {
  ...
  describe('HTML Parser', () => {
    it('imports HTML with scripts stripped', async () => {
      const html = '<html><head><script>alert("xss")</script></head><body><h1>Title</h1><p>Safe content</p></body></html>';
  ...
  describe('Database Integration', () => {
    it('all parsers write to database via SQLiteWriter', async () => {
      // Import via markdown
      const mdFiles: ParsedFile[] = [
        { path: 'md.md', content: '---\ntitle: MD Note\n---\nContent' },
      ];
      await orchestrator.import('markdown', JSON.stringify(mdFiles), {});

      // Import via CSV
      const csvFiles: ParsedFile[] = [
        { path: 'csv.csv', content: 'title,content\n"CSV Note","CSV Content"' },
      ];
