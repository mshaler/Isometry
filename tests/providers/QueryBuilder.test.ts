...
import type { CompiledAxis, CompiledDensity, CompiledFilter } from '../../src/providers/types';
...
describe('QueryBuilder.buildCardQuery()', () => {
	...
	it('with active axis returns ORDER BY from PAFVProvider', () => {
		...
		expect(result.sql).toBe('SELECT * FROM cards WHERE deleted_at IS NULL ORDER BY created_at DESC');
		expect(result.params).toEqual([]);
	});
	...
	it('with filter + axis returns both WHERE and ORDER BY', () => {
		...
		expect(result.sql).toBe('SELECT * FROM cards WHERE deleted_at IS NULL AND status = ? ORDER BY name ASC');
	...
	it('with limit and offset appends LIMIT and OFFSET clauses', () => {
		...
		expect(result.sql).toBe('SELECT * FROM cards WHERE deleted_at IS NULL LIMIT ? OFFSET ?');
...
describe('QueryBuilder.buildCountQuery()', () => {
	it('returns COUNT(*) with WHERE clause', () => {
		...
		expect(result.sql).toBe('SELECT COUNT(*) as count FROM cards WHERE deleted_at IS NULL');
		expect(result.params).toEqual([]);
	});
	...
	it('includes filter params in count query', () => {
		...
		expect(result.sql).toBe('SELECT COUNT(*) as count FROM cards WHERE deleted_at IS NULL AND folder = ?');
