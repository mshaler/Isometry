...
describe('AuditState', () => {
  ...
  describe('getChangeStatus', () => {
    it('returns "new" for inserted IDs', () => {
      audit.addImportResult(
        { insertedIds: ['card-1'], updatedIds: [], deletedIds: [] },
        'apple_notes'
      );
    ...
    it('returns "modified" for updated IDs', () => {
      audit.addImportResult(
        { insertedIds: [], updatedIds: ['card-2'], deletedIds: [] },
        'apple_notes'
      );
    ...
    it('returns "deleted" for deleted IDs', () => {
      audit.addImportResult(
        { insertedIds: [], updatedIds: [], deletedIds: ['card-3'] },
        'apple_notes'
      );
    ...
    it('priority: deleted > modified > new (deleted wins)', () => {
      audit.addImportResult(
        { insertedIds: ['card-x'], updatedIds: ['card-x'], deletedIds: ['card-x'] },
        'apple_notes'
      );
    ...
    it('priority: modified > new', () => {
      audit.addImportResult(
        { insertedIds: ['card-y'], updatedIds: ['card-y'], deletedIds: [] },
        'apple_notes'
      );
  ...
  describe('addImportResult', () => {
    it('accumulates across multiple imports (union, not replace)', () => {
      audit.addImportResult(
        { insertedIds: ['a'], updatedIds: ['b'], deletedIds: ['c'] },
        'apple_notes'
      );
      audit.addImportResult(
        { insertedIds: ['d'], updatedIds: ['e'], deletedIds: ['f'] },
        'markdown'
      );
    ...
    it('notifies subscribers', () => {
      ...
      audit.addImportResult(
        { insertedIds: ['a'], updatedIds: [], deletedIds: [] },
        'apple_notes'
      );
      expect(count).toBe(1);
    });
  ...
  describe('getDominantSource', () => {
    it('returns the most common source among given IDs', () => {
      audit.addImportResult(
        { insertedIds: ['a', 'b', 'c'], updatedIds: [], deletedIds: [] },
        'apple_notes'
      );
      audit.addImportResult(
        { insertedIds: ['d'], updatedIds: [], deletedIds: [] },
        'markdown'
      );
  ...
  describe('subscribe', () => {
    it('returns unsubscribe function', () => {
      let count = 0;
      const unsub = audit.subscribe(() => { count++; });
      audit.toggle();
      expect(count).toBe(1);
...
describe('audit-colors', () => {
  ...
  it('SOURCE_COLORS has all 9 source types', () => {
    const expected = [
      'apple_notes', 'markdown', 'csv', 'json', 'excel',
      'html', 'native_reminders', 'native_calendar', 'native_notes',
    ];
