...
describe('CalendarView — month granularity', () => {
  ...
  it('first day of month gets correct grid-column-start offset', () => {
    ...
    const firstDayCell = Array.from(dayCells).find(
      cell => (cell as HTMLElement).dataset['date'] === '2026-04-01'
  ...
  it('places cards in correct day cells by due_at date', () => {
    ...
    const cards = [
      makeCard({ id: 'card-a', name: 'Card A', due_at: `${targetDate}T09:00:00Z` }),
    ];
    view.render(cards);
