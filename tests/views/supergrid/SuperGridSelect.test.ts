...
import { classifyClickZone, SuperGridSelect } from '../../../src/views/supergrid/SuperGridSelect';
...
describe('SuperGridSelect lifecycle', () => {
	...
	it('attach() SVG has inset:0 style (via top/left/right/bottom or inset)', () => {
		...
		const hasInset =
			svg.style.inset === '0' || svg.style.inset === '0px' || (svg.style.top === '0px' && svg.style.left === '0px');
...
describe('SuperGridSelect lasso interactions', () => {
	...
	it('pointerup without metaKey calls selection.select(cardIds)', () => {
		...
		const getCellCardIds = vi.fn().mockImplementation((key: string) => (key === 'row1:col1' ? ['card-a'] : ['card-b']));
