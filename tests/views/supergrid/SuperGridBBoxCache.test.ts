...
import { rectsIntersect, SuperGridBBoxCache } from '../../../src/views/supergrid/SuperGridBBoxCache';
...
describe('SuperGridBBoxCache attach/detach lifecycle', () => {
	...
	it('after detach, cache is cleared and hitTest returns []', () => {
		...
		vi.spyOn(cell, 'getBoundingClientRect').mockReturnValue(new DOMRect(10, 10, 100, 50));
...
describe('SuperGridBBoxCache scheduleSnapshot', () => {
	...
	it('populates cache from .data-cell elements with dataset key', () => {
		...
		vi.spyOn(cell, 'getBoundingClientRect').mockReturnValue(new DOMRect(50, 100, 120, 60));
		gridEl.appendChild(cell);
	...
	it('populates cache with multiple cells', () => {
		...
		keys.forEach((key, i) => {
			...
			vi.spyOn(cell, 'getBoundingClientRect').mockReturnValue(new DOMRect(i * 100, 0, 90, 50));
			gridEl.appendChild(cell);
		});
	...
	it('overwrites previous cache entries on second scheduleSnapshot call', () => {
		...
		cell.className = 'data-cell';
		cell.dataset['key'] = 'rowA:colB';
		const spy = vi.spyOn(cell, 'getBoundingClientRect').mockReturnValue(new DOMRect(10, 10, 80, 40));
		gridEl.appendChild(cell);
	...
	it('ignores elements without dataset key', () => {
		...
		// No dataset['key'] set
		vi.spyOn(cellNoKey, 'getBoundingClientRect').mockReturnValue(new DOMRect(0, 0, 100, 50));
		...
		vi.spyOn(cellWithKey, 'getBoundingClientRect').mockReturnValue(new DOMRect(0, 0, 100, 50));
