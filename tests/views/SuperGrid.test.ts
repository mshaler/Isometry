...
describe('DYNM-01/DYNM-02 — SuperGrid axis DnD (grip handles + cross-dimension transpose)', () => {
	...
	it.skip('drop on col drop zone with row-origin payload calls provider.setColAxes with the dragged field appended', async () => {
		...
		const cells: CellDatum[] = [
			{ card_type: 'note', folder: 'A', status: 'todo', count: 1, card_ids: ['c1'], card_names: [] },
		];
		const { filter, coordinator } = makeDefaults([]);
		const { bridge } = makeMockBridge(cells);
		const view = new SuperGrid({
...
describe('DYNM-03 — Same-dimension axis reorder', () => {
	...
	it.skip('DYNM-03: col axis reorder [A,B,C] with A dragged to index 2 calls reorderColAxes(0, 2)', async () => {
		...
		// reorderColAxes called with (sourceIndex=0, targetIndex=2); setColAxes NOT called
	...
	it.skip('DYNM-03: row axis reorder calls reorderRowAxes(0, 1)', async () => {
		...
		// reorderRowAxes called with (0, 1); setRowAxes NOT called
		expect(reorderRowAxesSpy).toHaveBeenCalledWith(0, 1);
		expect(setRowAxesSpy).not.toHaveBeenCalled();
	...
	it.skip('DYNM-03: same-dimension reorder calls reorderColAxes — not setColAxes or setRowAxes', async () => {
		...
		// Only reorderColAxes called (not setColAxes or setRowAxes)
...
// ---------------------------------------------------------------------------
// ---------------------------------------------------------------------------
// Phase 31 Plan 02 — Visual drag UX: source dimming, insertion line, reorder method wiring
// ---------------------------------------------------------------------------

describe('Phase 31-02 — Visual drag UX: dimming, insertion line, reorder wiring', () => {
	let container: HTMLElement;
	...
	it.skip('same-dimension drop calls reorderColAxes (not setColAxes)', async () => {
		...
		// reorderColAxes should be called, NOT setColAxes
		expect(reorderColAxesSpy).toHaveBeenCalledWith(0, 1);
		expect(setColAxesSpy).not.toHaveBeenCalled();
	...
	it.skip('same-dimension row drop calls reorderRowAxes (not setRowAxes)', async () => {
		...
		const rowGrip = container.querySelector('.row-header .axis-grip') as HTMLElement;
		const rowDropZone = container.querySelector('[data-drop-zone="row"]') as HTMLElement;

		fireSameDimDrop(rowGrip, rowDropZone, 1);
	...
	it.skip('cross-dimension drop still calls setColAxes/setRowAxes (not reorder)', async () => {
		...
		expect(setColAxesSpy).toHaveBeenCalled();
		expect(setRowAxesSpy).toHaveBeenCalled();
		expect(reorderColAxesSpy).not.toHaveBeenCalled();
		expect(reorderRowAxesSpy).not.toHaveBeenCalled();
	...
	it.skip('same-dimension drop uses _lastReorderTargetIndex when dataset not set', async () => {
		...
		// Dataset-based drop (backward compat) still works via fireSameDimDrop
		fireSameDimDrop(firstGrip, colDropZone, 2);

		// Should call reorderColAxes with sourceIndex=0 and targetIndex=2
		expect(reorderColAxesSpy).toHaveBeenCalledWith(0, 2);
		expect(setColAxesSpy).not.toHaveBeenCalled();
...
describe('DYNM-04/DYNM-05 — Grid transition animation and axis persistence', () => {
	...
	it.skip('DYNM-05: after setColAxes is called by SuperGrid, provider returns the new axes from getStackedGroupBySQL', async () => {
		...
		const writtenColAxes = (provider.setColAxes as ReturnType<typeof vi.fn>).mock.calls[0]?.[0] as Array<{
			field: string;
		}>;
		...
		const writtenRowAxes = (provider.setRowAxes as ReturnType<typeof vi.fn>).mock.calls[0]?.[0] as Array<{
			field: string;
		}>;
...
describe('SIZE-01/02/03/04 — SuperGridSizer integration in SuperGrid', () => {
	...
	it.skip('SIZE-01: leaf column headers have .col-resize-handle child elements after render', async () => {
		const cells: CellDatum[] = [
			{ card_type: 'note', folder: 'A', count: 1, card_ids: ['c1'], card_names: [] },
			{ card_type: 'task', folder: 'B', count: 1, card_ids: ['c2'], card_names: [] },
		];
	...
	it.skip('SIZE-01: resize drag does NOT call bridge.superGridQuery() (pure CSS, no Worker round-trip)', async () => {
		const cells: CellDatum[] = [{ card_type: 'note', folder: 'A', count: 1, card_ids: ['c1'], card_names: [] }];
		const { provider, filter, bridge, superGridQuerySpy, coordinator } = makeDefaults(cells);
		const view = new SuperGrid({ provider, filter, bridge, coordinator });
		view.mount(container);
	...
	it.skip('SIZE-04: drag resize calls provider.setColWidths() after pointerup (Tier 2 persistence)', async () => {
		const cells: CellDatum[] = [{ card_type: 'note', folder: 'A', count: 1, card_ids: ['c1'], card_names: [] }];
		const { provider, filter, bridge, coordinator } = makeDefaults(cells);
	...
	it.skip('SIZE-04: initial colWidths loaded from provider.getColWidths() on mount', async () => {
		const cells: CellDatum[] = [{ card_type: 'note', folder: 'A', count: 1, card_ids: ['c1'], card_names: [] }];
	...
	it.skip('SIZE-04: data cells have data-col-key attribute set (enables dblclick auto-fit measurement)', async () => {
		const cells: CellDatum[] = [
			{ card_type: 'note', folder: 'A', count: 1, card_ids: ['c1'], card_names: [] },
			{ card_type: 'task', folder: 'B', count: 1, card_ids: ['c2'], card_names: [] },
		];
	...
	it.skip('_sizer.attach is called during mount (handles added after render)', async () => {
		const cells: CellDatum[] = [{ card_type: 'note', folder: 'A', count: 1, card_ids: ['c1'], card_names: [] }];
		const { provider, filter, bridge, coordinator } = makeDefaults(cells);
		const view = new SuperGrid({ provider, filter, bridge, coordinator });
		view.mount(container);
...
describe('SLCT — SuperSelect integration', () => {
	...
	it.skip('SLCT-03: Shift+click after plain click selects rectangular range', async () => {
		...
		dataCells[dataCells.length - 1]!.dispatchEvent(
...
describe('DENS-03 — View mode: spreadsheet and matrix', () => {
	...
	it.skip('Test 7: density toolbar contains a view mode control (select or segmented)', async () => {
		...
		const toolbar = container.querySelector('.supergrid-density-toolbar');
		expect(toolbar).not.toBeNull();
		const viewModeControl = toolbar?.querySelector(
			'select[data-control="view-mode"], button[data-view-mode], .view-mode-control',
		);
...
describe('Regression: Fix 3 — cell key encoding with compound key separators', () => {
	...
	it.skip('axis values containing colons produce correct cell keys', async () => {
		...
		const cells: CellDatum[] = [
			{ card_type: 'a:b', folder: 'work:personal', count: 2, card_ids: ['c1', 'c2'], card_names: [] },
		];
		const { provider, filter, coordinator } = makeDefaults(cells);
...
describe('SuperSort (Phase 23) — sort icon DOM and click handlers', () => {
	...
	function makeSortProvider(overrides: Partial<SuperGridProviderLike> = {}): {
		provider: SuperGridProviderLike;
		setSortOverridesSpy: ReturnType<typeof vi.fn>;
	} {
...
describe('SRCH-01/SRCH-02/SRCH-05 — SuperSearch: Cmd+F, debounce, immediate clear', () => {
	...
	it.skip('pressing Escape in search input does NOT propagate to document Escape handler (stopPropagation)', async () => {
		...
		let _documentEscapeFired = false;
		const docEscapeListener = (e: Event) => {
			if ((e as KeyboardEvent).key === 'Escape') {
				_documentEscapeFired = true;
			}
		};
...
describe('TIME-03 — Segmented granularity pills (replace <select>)', () => {
	...
	function makePillDensity(
		overrides: { axisGranularity?: TimeGranularity | null } = {},
	): SuperGridDensityLike & { setGranularity: ReturnType<typeof vi.fn>; _trigger: () => void } {
		const state = {
			axisGranularity: overrides.axisGranularity ?? null,
			hideEmpty: false,
...
describe('TIME-04/TIME-05 — Period selection via Cmd+click on time col headers', () => {
	...
	it.skip('TIME-04: Cmd+click on time axis col header calls filter.setAxisFilter with period key', async () => {
		...
		const colHeaders = container.querySelectorAll<HTMLElement>('.col-header');
		const jan = Array.from(colHeaders).find((h) =>
	...
	it.skip('TIME-05: Cmd+click two different period headers — setAxisFilter called with both keys', async () => {
		...
		const feb = Array.from(colHeaders).find((h) =>
			h.querySelector('.col-header-label')?.textContent?.includes('2026-02'),
		);
		expect(jan).not.toBeNull();
	...
	it.skip('TIME-04: Cmd+click on NON-time col header still calls selectionAdapter.addToSelection (SLCT-05 not regressed)', async () => {
		...
		const colHeaders = container.querySelectorAll<HTMLElement>('.col-header');
		const noteHeader = Array.from(colHeaders).find((h) =>
			h.querySelector('.col-header-label')?.textContent?.includes('note'),
		);
	...
	it.skip('TIME-04: selected period col header has teal accent background style', async () => {
		...
		const janAfter = Array.from(colHeadersAfter).find((h) =>
			h.querySelector('.col-header-label')?.textContent?.includes('2026-01'),
		);
...
describe('CARD-01/CARD-02 — SuperCard rendering', () => {
	...
	// CONV-06: Skipped -- DOM structure changed from CSS Grid to PivotGrid table layout.
	it.skip('CARD-01 matrix: data cell with count > 0 contains a .supergrid-card child element', async () => {
		const cells: CellDatum[] = [
			{ card_type: 'note', folder: 'A', count: 3, card_ids: ['c1', 'c2', 'c3'], card_names: [] },
		];
	...
	it.skip('CARD-01 matrix: SuperCard textContent equals the count as a string', async () => {
		const cells: CellDatum[] = [
			{
				card_type: 'note',
				folder: 'A',
				count: 7,
				card_ids: ['c1', 'c2', 'c3', 'c4', 'c5', 'c6', 'c7'],
				card_names: [],
			},
		];
		const { view } = makeViewWithDensity(cells, 'matrix');
	...
	it.skip('CARD-01 spreadsheet: value-first renders sg-cell-name as first child (no supergrid-card)', async () => {
		const cells: CellDatum[] = [
	...
	it.skip('CARD-02 style: SuperCard element has dashed border-style', async () => {
		const cells: CellDatum[] = [{ card_type: 'note', folder: 'A', count: 1, card_ids: ['c1'], card_names: [] }];
		const { view } = makeViewWithDensity(cells, 'matrix');
		view.mount(container);
		await new Promise((r) => setTimeout(r, 0));
		...
		expect(superCard).not.toBeNull();
		// Phase 76-03 RNDR-03: SuperCard styles moved from inline style.cssText to .supergrid-card CSS class.
		// jsdom does not apply CSS class rules, so we verify the class is present (CSS defines border:1px dashed ...).
		expect(superCard?.classList.contains('supergrid-card')).toBe(true);
		view.destroy();
	});
	...
	it.skip('CARD-02 style: SuperCard element has italic font-style', async () => {
		const cells: CellDatum[] = [{ card_type: 'note', folder: 'A', count: 1, card_ids: ['c1'], card_names: [] }];
		const { view } = makeViewWithDensity(cells, 'matrix');
		view.mount(container);
		await new Promise((r) => setTimeout(r, 0));
		...
		expect(superCard).not.toBeNull();
		// Phase 76-03 RNDR-03: SuperCard styles moved from inline style.cssText to .supergrid-card CSS class.
		// jsdom does not apply CSS class rules, so we verify the class is present (CSS defines font-style:italic).
		expect(superCard?.classList.contains('supergrid-card')).toBe(true);
		view.destroy();
	});
	...
	it.skip('CARD-02 attribute: SuperCard element has data-supercard="true" attribute', async () => {
		const cells: CellDatum[] = [{ card_type: 'note', folder: 'A', count: 1, card_ids: ['c1'], card_names: [] }];
		const { view } = makeViewWithDensity(cells, 'matrix');
		view.mount(container);
		await new Promise((r) => setTimeout(r, 0));
	...
	it.skip('CARD-02 heat map exclusion: parent .data-cell has empty backgroundColor when SuperCard is rendered', async () => {
		const cells: CellDatum[] = [
			{ card_type: 'note', folder: 'A', count: 5, card_ids: ['c1', 'c2', 'c3', 'c4', 'c5'], card_names: [] },
		];
		const { view } = makeViewWithDensity(cells, 'matrix');
	...
	it.skip('CARD-01: empty cells (count === 0) do NOT contain a .supergrid-card element', async () => {
		const cells: CellDatum[] = [
			{ card_type: 'note', folder: 'A', count: 1, card_ids: ['c1'], card_names: [] },
	...
	it.skip('CARD-02 spreadsheet: no SuperCard in spreadsheet mode (replaced by VFST-01 value-first)', async () => {
		const cells: CellDatum[] = [
			{ card_type: 'note', folder: 'A', count: 2, card_ids: ['c1', 'c2'], card_names: ['X', 'Y'] },
		];
		const { view } = makeViewWithDensity(cells, 'spreadsheet');
...
describe('CARD-03 — SuperCard tooltip', () => {
	...
	it.skip('CARD-03: tooltip contains a list item for each card ID', async () => {
		const cells: CellDatum[] = [
			{ card_type: 'note', folder: 'A', count: 2, card_ids: ['card-1', 'card-2'], card_names: [] },
		];
		const { view } = makeViewWithSuperCard(cells);
		view.mount(container);
		await new Promise((r) => setTimeout(r, 0));
...
describe('PLSH-05 -- Right-click context menu', () => {
	...
	it.skip('PLSH-05: clicking Sort ascending calls provider.setSortOverrides with asc', async () => {
		...
		const menu = container.querySelector('.sg-context-menu');
		const sortAscItem = Array.from(menu!.querySelectorAll('.sg-context-menu-item')).find((el) =>
	...
	it.skip('PLSH-05: clicking Sort descending calls provider.setSortOverrides with desc', async () => {
		...
		const menu = container.querySelector('.sg-context-menu');
		const sortDescItem = Array.from(menu!.querySelectorAll('.sg-context-menu-item')).find((el) =>
	...
	it.skip('PLSH-05: clicking Hide column triggers _fetchAndRender (bridge.superGridQuery called again)', async () => {
		...
		const menu = container.querySelector('.sg-context-menu');
		const hideItem = Array.from(menu!.querySelectorAll('.sg-context-menu-item')).find((el) =>
	...
	it.skip('PLSH-05: Hide column toggles to Show column on second right-click, restoring the column', async () => {
		...
		// Hide "note" column
		const noteHeader = Array.from(container.querySelectorAll('.col-header')).find(
			(h) => h.getAttribute('data-value') === 'note',
		) as HTMLElement;
		noteHeader.dispatchEvent(new MouseEvent('contextmenu', { bubbles: true, clientX: 100, clientY: 100 }));
		const hideItem = Array.from(
			container.querySelector('.sg-context-menu')!.querySelectorAll('.sg-context-menu-item'),
...
describe('SuperGrid compound keys (Phase 28)', () => {
	...
	// CONV-06: Skipped -- DOM structure changed from CSS Grid to PivotGrid table layout.
	it.skip('STAK-03: dataset key uses RECORD_SEP (\x1e) between row and col dimensions in single-axis config', async () => {
	...
	// CONV-06: Skipped -- DOM structure changed from CSS Grid to PivotGrid table layout.
	it.skip('STAK-03: 2-level col axes produce compound keys with UNIT_SEP (\x1f) within col dimension', async () => {
...
describe('RHDR — Multi-Level Row Headers (Phase 29)', () => {
	...
	it.skip('renders row headers at all stacking levels (RHDR-01)', async () => {
	...
	it.skip('parent row headers span child rows via grid-row (RHDR-03)', async () => {
		const provider = makeMultiRowAxisProvider(
			[{ field: 'card_type', direction: 'asc' }],
			[
				{ field: 'folder', direction: 'asc' },
	...
	it.skip('row header keys are unique across all levels (RHDR-04)', async () => {
		...
		const keys = Array.from(rowHeaders).map(
			(h) => h.dataset['key'] ?? `${h.dataset['level']}_${h.dataset['parentPath']}_${h.textContent}`,
		);
		const uniqueKeys = new Set(keys);
	...
	it.skip('single row axis still produces correct row headers (backward compat)', async () => {
...
describe('SuperGrid — collapse system (CLPS)', () => {
	...
	it.skip('CLPS-01: collapsing a header does not affect sibling headers', async () => {
		...
		const _secondHeader = level0Headers[1] as HTMLElement | undefined;
	...
	it.skip('CLPS-02: collapsed header in aggregate mode shows count badge on label', async () => {
		...
		const labelText =
			collapsedHeader?.querySelector('.col-header-label')?.textContent ?? collapsedHeader?.textContent ?? '';
	...
	it.skip('CLPS-02: collapsed group in aggregate mode renders a summary data cell with heat-map color', async () => {
		...
		// Count data cells before collapse
		const _cellsBefore = container.querySelectorAll('.data-cell').length;
	...
	it.skip('CLPS-05: collapse state persists through PAFVProvider across teardown/mount', async () => {
		...
		const { provider } = makeMockProvider({
			setCollapseState: vi.fn().mockImplementation((state) => {
				savedCollapseState = state;
			}),
	...
	it.skip('CLPS-06: clicking row header toggles collapse (plain click = collapse)', async () => {
		...
		const _rowHeadersBefore = container.querySelectorAll('.row-header').length;
...
describe('Phase 32 — deepest-wins aggregation', () => {
	...
	function _countNormalNonEmptyCells(el: HTMLElement): number {
		const allCells = el.querySelectorAll('.data-cell');
		let count = 0;
		for (const cell of allCells) {
			const datum = (cell as any).__data__;
	...
	// CONV-06: Skipped -- DOM structure changed from CSS Grid to PivotGrid table layout.
	it.skip('deepest-wins: level 0 only collapsed -> produces summary cells at level 0', async () => {
		...
		const summaryDatum = Array.from(allCells)
			.map((c) => (c as any).__data__)
			.find((d) => d?.isSummary);
	...
	// CONV-06: Skipped -- DOM structure changed from CSS Grid to PivotGrid table layout.
	it.skip('deepest-wins: level 1 only collapsed -> produces summary cells at level 1', async () => {
		const { view } = makeThreeAxisColSetup();
		await new Promise((r) => setTimeout(r, 0));
	...
	// CONV-06: Skipped -- DOM structure changed from CSS Grid to PivotGrid table layout.
	it.skip('deepest-wins: level 2 only collapsed -> produces summary cells at level 2', async () => {
		const { view } = makeThreeAxisColSetup();
		await new Promise((r) => setTimeout(r, 0));
	...
	// CONV-06: Skipped -- DOM structure changed from CSS Grid to PivotGrid table layout.
	it.skip('deepest-wins: levels 0+1 collapsed -> only level 1 produces summaries, level 0 suppressed', async () => {
		const { view } = makeThreeAxisColSetup();
		await new Promise((r) => setTimeout(r, 0));
	...
	// CONV-06: Skipped -- DOM structure changed from CSS Grid to PivotGrid table layout.
	it.skip('deepest-wins: levels 0+2 collapsed -> level 0 suppressed, level 2 produces summaries', async () => {
		const { view } = makeThreeAxisColSetup();
		await new Promise((r) => setTimeout(r, 0));
	...
	// CONV-06: Skipped -- DOM structure changed from CSS Grid to PivotGrid table layout.
	it.skip('deepest-wins: levels 1+2 collapsed -> only level 2 produces summaries, level 1 suppressed', async () => {
		const { view } = makeThreeAxisColSetup();
		await new Promise((r) => setTimeout(r, 0));
	...
	// CONV-06: Skipped -- DOM structure changed from CSS Grid to PivotGrid table layout.
	it.skip('deepest-wins: all three levels collapsed -> only level 2 produces summaries', async () => {
		const { view } = makeThreeAxisColSetup();
		await new Promise((r) => setTimeout(r, 0));
	...
	it.skip('deepest-wins: heat map / SuperCard rendering is consistent at all depth levels', async () => {
		...
		for (const cell of container.querySelectorAll('.data-cell')) {
			...
			if (datum?.isSummary && datum.count > 0) {
				...
				const hasContent = ((cell as HTMLElement).textContent?.trim().length ?? 0) > 0;
...
describe('Phase 32 — aggregate selection + auto-reconcile', () => {
	...
	it.skip('aggregate selection: click on summary cell selects all underlying cards', async () => {
		...
		const selectionAdapter: SuperGridSelectionLike = {
			select: vi.fn((ids: string[]) => {
				selectedCards.clear();
				ids.forEach((id) => selectedCards.add(id));
			}),
			addToSelection: vi.fn((ids: string[]) => {
				ids.forEach((id) => selectedCards.add(id));
			}),
	...
	it.skip('aggregate selection: auto-reconcile on collapse shows selection highlight on summary cell', async () => {
		...
		const hasHighlight = summaryEl.classList.contains('sg-selected') || summaryEl.style.outline.includes('#1a56f0');
	...
	it.skip('aggregate selection: auto-reconcile on expand re-shows individual cell selection', async () => {
		...
		for (const cell of dataCells) {
			...
			if (datum?.cardIds?.includes('c1')) {
				hasSelectedCell =
					(cell as HTMLElement).classList.contains('sg-selected') ||
...
describe('VFST-05 — value-first rendering regression', () => {
	...
	it.skip('comprehensive regression: single-card plain text, multi-card badge, empty cell, matrix SuperCard, badge tooltip', async () => {
		...
		const singleCell = Array.from(container.querySelectorAll<HTMLElement>('.data-cell')).find(
			(el) => el.dataset['rowKey'] === 'A' && el.dataset['colKey'] === 'note',
		);
		...
		const multiCell = Array.from(container.querySelectorAll<HTMLElement>('.data-cell')).find(
			(el) => el.dataset['rowKey'] === 'B' && el.dataset['colKey'] === 'note',
		);
...
describe('ACEL — Active Cell Focus', () => {
	...
	it.skip('ACEL-03: clicking a data cell adds sg-col--active-crosshair to matching column header', async () => {
		...
		const nonCrosshairHeaders = Array.from(colHeaders).filter((h) => !h.classList.contains('sg-col--active-crosshair'));
...
// ---------------------------------------------------------------------------
// Phase 84 WA1 — Aggregation + displayField wiring into superGridQuery
// ---------------------------------------------------------------------------

describe('Phase 84 WA1 — aggregation and displayField wiring', () => {
	let container: HTMLElement;
	...
	it.skip('passes aggregation=sum from provider into superGridQuery', async () => {
		...
		const { bridge, superGridQuerySpy } = makeMockBridge();
		const { coordinator } = makeMockCoordinator();
		// Provider returns 'sum' aggregation
		const { provider } = makeMockProvider({
		...
		const callArgs = superGridQuerySpy.mock.calls.at(-1)?.[0] as Record<string, unknown>;
		expect(callArgs?.['aggregation']).toBe('sum');
		view.destroy();
	});
	...
	it.skip('passes displayField from densityState when aggregation is non-count', async () => {
		...
		const { coordinator } = makeMockCoordinator();
		const { provider } = makeMockProvider({
			getAggregation: vi.fn().mockReturnValue('avg'),
		});

		const mockDensity: SuperGridDensityLike = {
			getState: () => ({
				axisGranularity: null,
				hideEmpty: false,
				viewMode: 'matrix' as const,
				regionConfig: null,
				displayField: 'priority' as import('../../src/providers/types').AxisField,
			}),
			setGranularity: vi.fn(),
			setHideEmpty: vi.fn(),
			setViewMode: vi.fn(),
		...
		const callArgs = superGridQuerySpy.mock.calls.at(-1)?.[0] as Record<string, unknown>;
		expect(callArgs?.['aggregation']).toBe('avg');
		expect(callArgs?.['displayField']).toBe('priority');
		view.destroy();
	});
	...
	it.skip('omits aggregation/displayField spread when aggregation is count (default)', async () => {
		...
		const callArgs = superGridQuerySpy.mock.calls.at(-1)?.[0] as Record<string, unknown>;
		expect(callArgs?.['aggregation']).toBeUndefined();
		expect(callArgs?.['displayField']).toBeUndefined();
		view.destroy();
	});
});
