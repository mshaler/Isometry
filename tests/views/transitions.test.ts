...
import { crossfadeTransition, shouldUseMorph } from '../../src/views/transitions';
import type { CardDatum, IView, PAFVProviderLike, WorkerBridgeLike } from '../../src/views/types';
...
function makeMockBridge(rows: Record<string, unknown>[] = []): WorkerBridgeLike {
	return {
		send: vi.fn((_type: string, _payload: unknown): Promise<unknown> => Promise.resolve({ rows })),
	};
}
...
function _makeCard(id: string): CardDatum {
	return {
		id,
		name: `Card ${id}`,
		folder: null,
