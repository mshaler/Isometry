...
describe('Database Foundation', () => {
	...
	describe('DB-02: Canonical schema', () => {
		...
		it('creates the cards_fts virtual table', () => {
			...
			const tables = db.exec("SELECT name FROM sqlite_master WHERE name='cards_fts'");
			expect(tables[0]?.values.flat()).toContain('cards_fts');
		});

		it('creates the ui_state table', () => {
			const tables = db.exec("SELECT name FROM sqlite_master WHERE type='table' AND name='ui_state'");
			expect(tables[0]?.values.flat()).toContain('ui_state');
		});

		it('has idx_cards_type index', () => {
			const idx = db.exec("SELECT name FROM sqlite_master WHERE type='index' AND name='idx_cards_type'");
			expect(idx[0]?.values.flat()).toContain('idx_cards_type');
		});

		it('has idx_cards_folder index', () => {
			const idx = db.exec("SELECT name FROM sqlite_master WHERE type='index' AND name='idx_cards_folder'");
			expect(idx[0]?.values.flat()).toContain('idx_cards_folder');
		});

		it('has idx_cards_status index', () => {
			const idx = db.exec("SELECT name FROM sqlite_master WHERE type='index' AND name='idx_cards_status'");
			expect(idx[0]?.values.flat()).toContain('idx_cards_status');
		});

		it('has idx_cards_created index', () => {
			const idx = db.exec("SELECT name FROM sqlite_master WHERE type='index' AND name='idx_cards_created'");
			expect(idx[0]?.values.flat()).toContain('idx_cards_created');
		});

		it('has idx_cards_modified index', () => {
			const idx = db.exec("SELECT name FROM sqlite_master WHERE type='index' AND name='idx_cards_modified'");
			expect(idx[0]?.values.flat()).toContain('idx_cards_modified');
		});

		it('has idx_cards_source unique index', () => {
			const idx = db.exec("SELECT name FROM sqlite_master WHERE type='index' AND name='idx_cards_source'");
			expect(idx[0]?.values.flat()).toContain('idx_cards_source');
		});

		it('has idx_conn_source index', () => {
			const idx = db.exec("SELECT name FROM sqlite_master WHERE type='index' AND name='idx_conn_source'");
			expect(idx[0]?.values.flat()).toContain('idx_conn_source');
		});

		it('has idx_conn_target index', () => {
			const idx = db.exec("SELECT name FROM sqlite_master WHERE type='index' AND name='idx_conn_target'");
			expect(idx[0]?.values.flat()).toContain('idx_conn_target');
		});

		it('has idx_conn_via index', () => {
			const idx = db.exec("SELECT name FROM sqlite_master WHERE type='index' AND name='idx_conn_via'");
		...
		it('enforces UNIQUE constraint on connections (source_id, target_id, via_card_id, label)', () => {
			...
			db.run("INSERT INTO connections(id, source_id, target_id, via_card_id, label) VALUES(?, ?, ?, ?, 'links')", [
				uid('conn'),
				cardA,
				cardB,
				cardVia,
	...
	describe('DB-03: FTS5 sync triggers', () => {
		it('trigger cards_fts_ai (AFTER INSERT) exists', () => {
			const triggers = db.exec("SELECT name FROM sqlite_master WHERE type='trigger' AND name='cards_fts_ai'");
			expect(triggers[0]?.values.flat()).toContain('cards_fts_ai');
		});

		it('trigger cards_fts_ad (AFTER DELETE) exists', () => {
			const triggers = db.exec("SELECT name FROM sqlite_master WHERE type='trigger' AND name='cards_fts_ad'");
			expect(triggers[0]?.values.flat()).toContain('cards_fts_ad');
		});

		it('trigger cards_fts_au (AFTER UPDATE) exists', () => {
			const triggers = db.exec("SELECT name FROM sqlite_master WHERE type='trigger' AND name='cards_fts_au'");
			expect(triggers[0]?.values.flat()).toContain('cards_fts_au');
		});

		it('has exactly three FTS triggers (no extras)', () => {
			const triggers = db.exec("SELECT name FROM sqlite_master WHERE type='trigger' AND name LIKE 'cards_fts_%'");
	...
	describe('DB-06: Foreign keys', () => {
		...
		it('cascade delete removes connections when source card is deleted', () => {
			...
			db.run("INSERT INTO connections(id, source_id, target_id, label) VALUES(?, ?, ?, 'links')", [
				connId,
				cardA,
				cardB,
			]);
		...
		it('deleting the via card sets via_card_id to NULL (SET NULL, not cascade)', () => {
			...
			db.run('INSERT INTO cards(id, name) VALUES(?, ?)', [cardB, 'Card B']);
			db.run('INSERT INTO cards(id, name) VALUES(?, ?)', [cardVia, 'Via Card']);
			db.run("INSERT INTO connections(id, source_id, target_id, via_card_id, label) VALUES(?, ?, ?, ?, 'through')", [
				connId,
				cardA,
				cardB,
				cardVia,
			...
			const result = db.exec('SELECT id, via_card_id FROM connections WHERE id = ?', [connId]);
