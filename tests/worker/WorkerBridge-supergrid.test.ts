...
import type { WorkerRequest, CellDatum } from '../../src/worker/protocol';
...
describe('WorkerBridge.superGridQuery()', () => {
  ...
  it('returns CellDatum[] from Worker response', async () => {
    ...
    const expectedCells: CellDatum[] = [
      { status: 'active', folder: 'Inbox', count: 3, card_ids: ['a', 'b', 'c'] },
    ];

    mockWorker.setMessageHandler((request: WorkerRequest) => {
      mockWorker.simulateMessage(
        createSuccessResponse<'supergrid:query'>(request.id, { cells: expectedCells })
      );
    });
  ...
  it('4 synchronous calls produce exactly 1 Worker postMessage', async () => {
    ...
    mockWorker.setMessageHandler((request: WorkerRequest) => {
      mockWorker.simulateMessage(
        createSuccessResponse<'supergrid:query'>(request.id, { cells: [] })
      );
    });
    ...
    const superGridCalls = () =>
      postSpy.mock.calls.filter(
        (call) => call[0] && (call[0] as WorkerRequest).type === 'supergrid:query'
      );
  ...
  it('latest-wins: only 4th caller resolves; first 3 are abandoned', async () => {
    ...
    const mockWorker = (bridge as unknown as { worker: MockWorker }).worker;
    const expectedCells: CellDatum[] = [
      { status: 'done', count: 1, card_ids: ['z'] },
    ];
...
describe('WorkerBridge.distinctValues()', () => {
  ...
  it('returns string[] from Worker response', async () => {
    ...
    mockWorker.setMessageHandler((request: WorkerRequest) => {
      mockWorker.simulateMessage(
        createSuccessResponse<'db:distinct-values'>(request.id, { values: expectedValues })
      );
    });
  ...
  it('forwards WHERE + params in payload', async () => {
    ...
    mockWorker.setMessageHandler((request: WorkerRequest) => {
      ...
      mockWorker.simulateMessage(
        createSuccessResponse<'db:distinct-values'>(request.id, { values: ['active'] })
      );
    });
  ...
  it('omits where and params when not provided', async () => {
    ...
    mockWorker.setMessageHandler((request: WorkerRequest) => {
      capturedPayload = request.payload;
      mockWorker.simulateMessage(
        createSuccessResponse<'db:distinct-values'>(request.id, { values: [] })
      );
    });
