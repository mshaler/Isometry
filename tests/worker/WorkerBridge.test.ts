...
import type { Card, WorkerRequest } from '../../src/worker/protocol';
import {
	createErrorResponse,
...
describe('WorkerBridge', () => {
	...
	describe('Request/Response Correlation', () => {
		it('should generate unique correlation IDs', async () => {
			...
			mockWorker.setMessageHandler((request) => {
				ids.add(request.id);
				// Respond immediately
				mockWorker.simulateMessage(createSuccessResponse<'card:list'>(request.id, []));
			});

			// Send multiple requests
			await Promise.all([bridge.listCards(), bridge.listCards(), bridge.listCards()]);
	...
	describe('Timeout Handling', () => {
		...
		it('should not reject late responses after timeout', async () => {
			...
			const mockWorker = (bridge as unknown as { worker: MockWorker }).worker;
			let _capturedId: string | null = null;

			mockWorker.setMessageHandler((request) => {
				_capturedId = request.id;
