...
import { createCardInput, createConnectionInput, MINIMAL_CARD_INPUT } from './fixtures';
...
describe('WorkerBridge Integration', () => {
	...
	describe('Initialization', () => {
		...
		it('should handle requests sent before explicitly awaiting isReady (queue replay contract)', async () => {
			...
			const createCardPromise = bridge.createCard(createCardInput({ name: 'Queue Replay Test Card' }));
	...
	describe('Connection CRUD', () => {
		...
		it('should create a connection between cards', async () => {
			const conn: Connection = await bridge.createConnection(createConnectionInput(sourceId, targetId));
