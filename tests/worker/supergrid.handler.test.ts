...
import { handleDistinctValues, handleSuperGridQuery } from '../../src/worker/handlers/supergrid.handler';
...
describe('handleSuperGridQuery', () => {
	it('returns cells with card_ids split from comma-string to string[]', () => {
		...
		const db = createSimpleMockDb([{ card_type: 'note', folder: 'Inbox', count: 2, card_ids: 'id1,id2' }]);
	...
	it('returns single cell with total count when both axes are empty', () => {
		const db = createSimpleMockDb([{ NULL: null, count: 5, card_ids: 'id1,id2,id3,id4,id5' }]);
	...
	it('handles null card_ids (empty group) gracefully', () => {
		const db = createSimpleMockDb([{ card_type: 'note', count: 0, card_ids: null }]);
	...
	it('handles empty string card_ids gracefully', () => {
		const db = createSimpleMockDb([{ card_type: 'note', count: 0, card_ids: '' }]);
	...
	it('without searchTerm: does NOT add matchedCardIds to cells', () => {
		const db = createSimpleMockDb([{ card_type: 'note', count: 1, card_ids: 'id1' }]);
...
describe('handleDistinctValues', () => {
	it('returns sorted string values for valid column', () => {
		...
		expect(db.exec).toHaveBeenCalledWith(expect.stringContaining('ORDER BY folder ASC'), expect.any(Array));
	});
	...
	it('respects WHERE filter and params', () => {
		...
		expect(db.exec).toHaveBeenCalledWith(expect.stringContaining('AND folder = ?'), ['Inbox']);
	});
