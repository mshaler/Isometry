...
describe('HTMLParser - Stripping and Metadata', () => {
	...
	beforeEach(() => {
		...
		fixtureHtml = readFileSync(join(__dirname, '../fixtures/html-with-scripts.html'), 'utf-8');
	});
...
describe('HTMLParser - Markdown Conversion', () => {
	...
	beforeEach(() => {
		parser = new HTMLParser();
		articleHtml = readFileSync(join(__dirname, '../fixtures/html-article.html'), 'utf-8');
	});
...
describe('HTMLParser - Full Parse with Author and Resources', () => {
	...
	beforeEach(() => {
		parser = new HTMLParser();
		webClippingHtml = readFileSync(join(__dirname, '../fixtures/html-web-clipping.html'), 'utf-8');
	});
