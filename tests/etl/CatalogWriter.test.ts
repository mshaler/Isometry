...
describe('CatalogWriter', () => {
	...
	it('reuses existing import source on subsequent runs', () => {
		...
		const sources = db.prepare<{ id: string }>('SELECT id FROM import_sources').all();

		expect(sources.length).toBe(1);

		// Should have two runs
		const runs = db.prepare<{ id: string }>('SELECT id FROM import_runs').all();
	...
	it('handles empty errors_detail array', () => {
		...
		const run = db
			.prepare<{ errors_json: string | null }>('SELECT errors_json FROM import_runs WHERE id = ?')
			.all(runId);
