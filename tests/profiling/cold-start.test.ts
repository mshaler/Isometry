...
it(
	'cold-start decomposition: logs WASM init / DB create / schema apply breakdown',
	async () => {
		clearTraces();
		...
		if (dbCreate.length === 0) {
			console.warn(
				'WARNING: db:instance:create trace missing — check PerfTrace markers in Database.initialize()',
			);
		}
		if (schemaApply.length === 0) {
			console.warn(
				'WARNING: db:schema:apply trace missing — check PerfTrace markers in Database.initialize()',
			);
		}
	},
	30_000,
